...
class TestResponseQualityE2E:
    ...
    def test_contextual_responses_e2e(self, service_url):
        ...
        test_cases = [
            ...
            },
            {
                ...
                "expected_keywords": ["Weighted utensils", "Built-up handles", "Plate guards"],
                "expected_category": "contextual_override",
                "expected_method": "contextual_analysis"
            }
        ]
    ...
    def test_ml_classification_e2e(self, service_url):
        ...
        assert any(term in data["response"].lower() for term in 
                  ["balance", "exercise", "walking", "tai chi"])
    ...
    def test_stats_endpoint_e2e(self, service_url):
        ...
        requests.post(
            f"{service_url}/chat",
            json={"message": "Test query 1"},
            timeout=10
        )
        requests.post(
            f"{service_url}/chat",
            json={"message": "Test query 2"},
            timeout=10
        )
...
class TestResponseQualityValidation:
    ...
    def test_medical_disclaimer_present(self, service_url):
        ...
        for query in medical_queries:
            ...
            assert any(term in response_text for term in 
                      ["consult", "healthcare provider", "professional", "⚠️"])
    ...
    def test_crisis_resources_complete(self, service_url):
        ...
        for query in crisis_queries:
            ...
            assert "988" in data["response"]
            assert any(term in data["response"] for term in 
                      ["Crisis", "CRISIS", "emergency", "immediately"])
