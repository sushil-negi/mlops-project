    ...
    def test_classification_accuracy(self, engine, query, expected_category):
        ...
        if "hurt myself" in query.lower() or "suicide" in query.lower():
            ...
            if response['method'] == 'contextual_analysis':
                ...
                assert expected_category in response['category'] or \
                       response['category'] in ['contextual_override', expected_category]
...
class TestResponseQualityMetrics:
    ...
    def test_response_safety_warnings(self, engine):
        ...
        assert any(term in response['response'].lower() for term in 
                  ['professional', 'healthcare provider', 'therapist', 'consult'])
    
    def test_response_specificity(self, engine):
        """Test that responses are specific to the query"""
        # Test eating equipment
        eating_response = engine.generate_response("I need adaptive equipment for eating")
        assert "eating" in eating_response['response'].lower() or \
               "utensils" in eating_response['response'].lower() or \
               "weighted" in eating_response['response'].lower()
        
        # Test exercise
        exercise_response = engine.generate_response("What exercises help seniors?")
        assert any(term in exercise_response['response'].lower() for term in 
                  ['exercise', 'walking', 'balance', 'strength', 'tai chi'])
        
        # Test medication
        med_response = engine.generate_response("How to remember medications?")
        assert any(term in med_response['response'].lower() for term in 
                  ['medication', 'pill', 'reminder', 'organizer'])
