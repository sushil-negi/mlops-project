    ...
    class BasicHealthcareEngine:
        ...
        def get_stats(self):
            return {
                'model_loaded': True,
                'categories': 4,
                'category_list': ['adl', 'senior_care', 'mental_health', 'respite_care'],
                'total_responses': 12,
                'cache_size': 0,
...
class HealthcareAIHandler(BaseHTTPRequestHandler):
    ...
    def handle_chat_request(self):
        ...
        try:
            ...
            if content_length == 0:
                self.send_json_response({"error": "No message provided"}, status_code=400)
                return
            ...
            self.send_json_response({"error": f"Processing error: {str(e)}"}, status_code=500)
