...
class HealthcareAIEngine:
    ...
    def __init__(self, use_llm=True, model_name="microsoft/DialoGPT-small"):
        ...
        if use_llm:
            try:
                ...
                logger.warning(f"Failed to load LLM: {e}. Falling back to knowledge base only.")
                self.use_llm = False
    ...
    def _load_knowledge_base(self) -> Dict:
        ...
        try:
            with open('/Users/snegi/Documents/github/mlops-project/data/combined_healthcare_training_data.jsonl', 'r') as f:
                # Skip header lines
                next(f)
                next(f)
                ...
                for i, line in enumerate(f):
                    ...
                    try:
                        ...
                        if 'messages' in conv and len(conv['messages']) >= 2:
                            ...
                            knowledge['responses_by_category'][category].append(assistant_msg)
                            
                            # Collect crisis responses
                            if any(word in user_msg.lower() for word in ['suicide', 'crisis', 'emergency']):
                                knowledge['crisis_responses'].append(assistant_msg)
                    except:
                        continue
                        
            logger.info(f"Loaded {sum(len(v) for v in knowledge['conversations'].values())} conversations")
        except Exception as e:
            logger.error(f"Error loading knowledge base: {e}")
            
        # Add comprehensive category mappings
        knowledge['category_keywords'] = {
            'adl': ['mobility', 'walking', 'transfer', 'wheelchair', 'balance', 'falling', 'bathing', 
                   'dressing', 'eating', 'toileting', 'daily activities', 'independence', 'adaptive'],
            'senior_care': ['elderly', 'senior', 'aging', 'parent', 'grandfather', 'grandmother', 
                           'lonely', 'isolation', 'memory', 'medication', 'aging in place', 'geriatric'],
            'mental_health': ['anxiety', 'depression', 'stress', 'panic', 'worried', 'sad', 'overwhelmed', 
                             'mental health', 'therapy', 'counseling', 'emotional', 'mood'],
            'respite_care': ['caregiver', 'exhausted', 'break', 'respite', 'burnout', 'caring for', 
                            'relief', 'temporary care', 'support', 'overwhelmed'],
            'disabilities': ['disability', 'wheelchair', 'adaptive', 'accessibility', 'accommodation', 
                            'assistive', 'impairment', 'special needs', 'inclusion']
        }
    ...
    def _detect_category(self, text: str) -> str:
        ...
        crisis_words = ['suicide', 'kill myself', 'hurt myself', 'want to die', 'end it all']
    ...
    def _get_similar_conversation(self, user_input: str, category: str) -> Optional[Dict]:
...
Healthcare Assistant: """
            ...
            if any(word in user_input.lower() for word in ['medical', 'health', 'treatment', 'diagnosis']):
    ...
    def _create_contextual_response(self, user_input: str, category: str) -> str:
        ...
        if similar:
            ...
            if "my" in user_input.lower():
                base_response = base_response.replace("For patients", "For your situation")
    ...
    def get_conversation_stats(self) -> Dict:
        ...
        return {
            ...
            'loaded_conversations': sum(len(v) for v in self.knowledge_base['conversations'].values()),
