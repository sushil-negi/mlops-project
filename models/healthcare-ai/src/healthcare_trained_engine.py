...
class HealthcareTrainedEngine:
    ...
    def _load_model(self):
        ...
        try:
            if os.path.exists(self.model_path):
                ...
                logger.info(f"Loaded trained model with {len(self.category_mapping)} categories")
    ...
    def generate_response(self, user_input: str) -> Dict:
        ...
        try:
            ...
            if category_responses:
                ...
                response = self._select_contextual_response(user_input, category_responses)
                method = 'ml_model'
            else:
                # Fallback response
    ...
    def _is_crisis(self, text: str) -> bool:
        ...
        crisis_words = ['suicide', 'kill myself', 'hurt myself', 'want to die', 'end it all', 'harm myself']
    ...
    def _select_contextual_response(self, user_input: str, responses: List[str]) -> str:
        ...
        mobility_keywords = ['bed', 'chair', 'standing', 'walking', 'transfer', 'wheelchair', 'mobility', 'movement', 'getting up', 'sitting down']
        medication_keywords = ['medication', 'pills', 'medicine', 'prescription', 'drug', 'dose', 'reminder', 'memory']
        caregiver_keywords = ['caring', 'caregiver', 'spouse', 'parent', 'family', 'overwhelmed', 'tired', 'stress', 'burden']
        crisis_keywords = ['help', 'emergency', 'urgent', 'crisis', 'immediate', 'serious']
        daily_living_keywords = ['bathing', 'dressing', 'eating', 'toileting', 'grooming', 'cooking', 'cleaning']
        ...
        for response in responses:
            ...
            if any(keyword in user_lower for keyword in mobility_keywords):
                if any(keyword in response_lower for keyword in ['transfer', 'mobility', 'wheelchair', 'bed', 'chair', 'standing']):
                    score += 10
            
            # Exercise-specific scoring
            if any(keyword in user_lower for keyword in ['exercise', 'exercises', 'workout', 'fitness', 'physical activity']):
                if any(keyword in response_lower for keyword in ['exercise', 'balance', 'strength', 'tai chi', 'walking', 'movement']):
                    score += 15
            
            if any(keyword in user_lower for keyword in medication_keywords):
                if any(keyword in response_lower for keyword in ['medication', 'pill', 'reminder', 'organizer', 'alarm']):
                    score += 10
            
            if any(keyword in user_lower for keyword in caregiver_keywords):
                if any(keyword in response_lower for keyword in ['respite', 'support', 'care', 'break', 'help']):
                    score += 10
            
            if any(keyword in user_lower for keyword in daily_living_keywords):
                if any(keyword in response_lower for keyword in ['adaptive', 'equipment', 'aids', 'assistance']):
                    score += 10
    ...
    def _check_specific_scenarios(self, user_input: str) -> Optional[str]:
        ...
        if any(term in user_lower for term in ['father', 'mother', 'parent', 'elderly']) and \
           any(term in user_lower for term in ['bed', 'getting up', 'standing', 'trouble', 'difficulty', 'struggle']):
        ...
        if 'medication' in user_lower and any(term in user_lower for term in ['memory', 'forget', 'remember', 'reminder']):
        ...
        if any(term in user_lower for term in ['overwhelmed', 'exhausted', 'burned out']) and \
           any(term in user_lower for term in ['caring', 'caregiver', 'spouse', 'husband', 'wife', 'partner']):
        ...
        if 'wheelchair' in user_lower and any(term in user_lower for term in ['transfer', 'moving', 'getting in', 'getting out']):
        ...
        if any(term in user_lower for term in ['depressed', 'depression', 'sad', 'hopeless', 'down']):
        ...
        if any(term in user_lower for term in ['anxious', 'anxiety', 'worried', 'panic', 'nervous']):
        ...
        if ('exercise' in user_lower or 'exercises' in user_lower) and ('senior' in user_lower or 'elderly' in user_lower or 'older' in user_lower):
        ...
        if 'eating' in user_lower and any(term in user_lower for term in ['equipment', 'adaptive', 'help', 'aids', 'tools', 'need']):
    ...
    def _generate_contextual_response(self, user_input: str, base_response: str) -> str:
        ...
        if 'father' in user_lower and ('bed' in user_lower or 'getting up' in user_lower):
        ...
        elif 'overwhelmed' in user_lower and ('spouse' in user_lower or 'caring' in user_lower):
        ...
        elif 'depression' in user_lower or 'sad' in user_lower or 'hopeless' in user_lower:
            return "Depression affects many people and is treatable. Steps to take: 1) Contact your doctor or mental health professional, 2) Maintain daily routines, 3) Stay connected with supportive people, 4) Engage in activities you usually enjoy, 5) Consider counseling or therapy. ⚠️ If having thoughts of self-harm, call 988 (Suicide & Crisis Lifeline) immediately."
        
        elif 'anxiety' in user_lower or 'worried' in user_lower or 'panic' in user_lower:
    ...
    def _get_fallback_response(self, category: str) -> str:
        ...
        return fallbacks.get(category, "I can help with healthcare questions. Please provide more details about your specific needs. ⚠️ For medical advice, consult healthcare professionals.")
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'category_list': list(self.category_mapping.values()) if self.category_mapping else [],
            'total_responses': sum(len(responses) for responses in self.healthcare_responses.values()) if self.healthcare_responses else 0,
