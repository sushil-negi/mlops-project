...
def generate_coverage_report():
    ...
    subprocess.run([sys.executable, "-m", "coverage", "report"], cwd=project_root)
...
def main():
    ...
    parser.add_argument(
        "--skip-web", action="store_true", help="Skip web interface tests"
    )
    ...
    if not service_running and (
        args.integration
        or args.e2e
        or (not any([args.unit, args.integration, args.e2e]))
    ):
