...
def main():
    ...
    # Summary
    logger.info("📋 Summary:")
    logger.info(f"  - Core services: {'✅ All healthy' if all_healthy else '❌ Some services down'}")
    logger.info(f"  - Prometheus targets: {len(targets['active'])} active, {len(targets['down'])} down")
    logger.info(f"  - Healthcare metrics: {'✅ Available' if healthcare_metrics_ok else '❌ Not available'}")
    logger.info(f"  - Grafana: {'✅ Connected' if grafana_ok else '❌ Not connected'}")
    logger.info(f"  - Alertmanager: {'✅ Running' if alertmanager_ok else '❌ Not running'}")
