...
class SystemMetricsLogger:
    ...
    def log_to_mlflow(self, step=None):
        ...
        mlflow.log_metric("system/cpu_percent", current_metrics['cpu_percent'], step=step)
        mlflow.log_metric("system/memory_percent", current_metrics['memory_percent'], step=step)
        mlflow.log_metric("system/memory_used_gb", current_metrics['memory_used_gb'], step=step)
        mlflow.log_metric("system/disk_usage_percent", current_metrics['disk_usage_percent'], step=step)
        
        # Log GPU metrics
        for gpu_data in current_metrics['gpu_metrics']:
            gpu_id = gpu_data['gpu_id']
            mlflow.log_metric(f"system/gpu_{gpu_id}_utilization", gpu_data['gpu_utilization'], step=step)
            mlflow.log_metric(f"system/gpu_{gpu_id}_memory_percent", gpu_data['memory_used_percent'], step=step)
            mlflow.log_metric(f"system/gpu_{gpu_id}_memory_used_mb", gpu_data['memory_used_mb'], step=step)
            mlflow.log_metric(f"system/gpu_{gpu_id}_temperature", gpu_data['temperature'], step=step)
    ...
    def create_system_metrics_plots(self):
        ...
        time_minutes = [(t - start_time) / 60 for t in self.metrics_history['timestamp']]
        ...
        # Memory usage
        ax2.plot(time_minutes, self.metrics_history['memory_percent'], 'r-', label='Memory %')
        ax2.plot(time_minutes, self.metrics_history['memory_used_gb'], 'g-', label='Memory Used (GB)')
        ...
        if self.gpu_count > 0 and len(self.metrics_history['gpu_metrics']) > 0:
            ...
            for i in range(min(self.gpu_count, 2)):  # Plot first 2 GPUs
                ...
                for gpu_data_list in self.metrics_history['gpu_metrics']:
                    gpu_data = next((g for g in gpu_data_list if g['gpu_id'] == i), None)
                    if gpu_data:
                ...
                line1 = ax_util.plot(time_minutes, gpu_util, 'b-', label=f'GPU {i} Utilization %')
                line2 = ax_util_twin.plot(time_minutes, gpu_temp, 'r--', label=f'GPU {i} Temp (°C)')
...
class EnhancedTrainingDemo:
    ...
    def train_with_comprehensive_logging(self):
        ...
        with mlflow.start_run(run_name=f"enhanced-training-{datetime.now().strftime('%Y%m%d-%H%M%S')}"):
            ...
            model_summary = {
                ...
                "training_time_minutes": len(self.system_logger.metrics_history['timestamp']) * 10 / 60  # Approximate
            }
            ...
            print(f"\n📊 Final Results:")
            print(f"  Best Validation Accuracy: {model_summary['best_val_accuracy']:.2f}%")
            print(f"  Final Training Accuracy: {model_summary['final_train_accuracy']:.2f}%")
            ...
            print(f"\n🔗 View Results:")
            print(f"  MLflow UI: http://localhost:5001/#/experiments/{mlflow.get_experiment_by_name(experiment_name).experiment_id}/runs/{run_id}")
    ...
    def create_and_log_training_plots(self):
        ...
        ax4.bar(['Train Acc', 'Val Acc'], [train_accs[-1], val_accs[-1]], 
        ...
        for i, (label, value) in enumerate(zip(['Train Acc', 'Val Acc'], [train_accs[-1], val_accs[-1]])):
