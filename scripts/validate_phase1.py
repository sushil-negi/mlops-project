    ...
    try:
        ...
        for query, expected_category in test_cases:
            ...
            if result.get('category') != expected_category:
                print(f"   Category mismatch: {query} -> {result.get('category')} (expected {expected_category})")
                return False
...
def test_data_drift_detector():
    ...
    try:
        ...
        result = subprocess.run([
            'python3', 'scripts/data_drift_detector.py',
            '--reference-data', temp_file,
            '--hours', '1'
        ...
        return result.returncode == 0 or result.returncode == 1  # 1 = drift detected (OK)
