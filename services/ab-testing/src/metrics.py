...
from .models import MetricSnapshot, ExperimentMetrics as ExperimentMetricsModel
        ...
        try:
            ...
            if self.redis_client:
                snapshot = MetricSnapshot(
                    ...
                    response_time_p50=metrics.get('response_time'),  # Individual response time
                ...
                key = f"metrics:{experiment_id}:{model}:{timestamp.strftime('%Y%m%d%H')}"
            ...
            logger.error("Failed to log metrics", experiment_id=experiment_id, error=str(e))
            raise
    
    async def get_experiment_metrics(self, experiment_id: str, hours_back: int = 24) -> Dict:
        ...
        try:
            ...
            metrics_data = {
                'model_a': {'samples': [], 'accuracy': [], 'empathy_score': [], 
                           'response_time': [], 'crisis_detection_rate': []},
                'model_b': {'samples': [], 'accuracy': [], 'empathy_score': [], 
                           'response_time': [], 'crisis_detection_rate': []}
            }
            ...
            for key in keys:
                ...
                for raw_metric in raw_metrics:
                    try:
                        ...
                        model_key = 'model_a' if snapshot.model.endswith('_a') else 'model_b'
                        
                        # Add to aggregated data
                        metrics_data[model_key]['samples'].append(snapshot.dict())
                        
                        if snapshot.accuracy is not None:
                            metrics_data[model_key]['accuracy'].append(snapshot.accuracy)
                        
                        if snapshot.empathy_score is not None:
                            metrics_data[model_key]['empathy_score'].append(snapshot.empathy_score)
                        
                        if snapshot.response_time_p50 is not None:
                            metrics_data[model_key]['response_time'].append(snapshot.response_time_p50)
                        
                        if snapshot.crisis_detection_rate is not None:
                            metrics_data[model_key]['crisis_detection_rate'].append(snapshot.crisis_detection_rate)
            ...
            for model in ['model_a', 'model_b']:
                ...
                for metric in ['accuracy', 'empathy_score', 'response_time', 'crisis_detection_rate']:
                    values = data[metric]
                    if values:
            ...
            logger.error("Failed to get experiment metrics", experiment_id=experiment_id, error=str(e))
            return {"error": str(e)}
    ...
    async def calculate_statistical_readiness(self, experiment_id: str) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'ready_for_analysis': model_a_samples >= min_samples_needed and model_b_samples >= min_samples_needed,
                'has_recommended_power': model_a_samples >= recommended_samples and model_b_samples >= recommended_samples,
                'completion_percentage': min(
                    (model_a_samples + model_b_samples) / (2 * recommended_samples),
                    1.0
                ) * 100
            }
