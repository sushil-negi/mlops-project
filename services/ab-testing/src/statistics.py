...
class ABTestAnalyzer:
    ...
    def analyze_experiment(self, experiment_data: Dict) -> Dict:
        ...
        try:
            ...
            # Analyze each metric
            metrics_to_analyze = ['accuracy', 'empathy_score', 'response_time', 'crisis_detection_rate']
            ...
            results["overall_recommendation"] = self._generate_recommendation(results["metrics"])
            
            # Calculate statistical power
            results["statistical_power"] = self._calculate_statistical_power(results["metrics"])
    ...
    def _analyze_metric(self, metric_name: str, values_a: List[float], values_b: List[float]) -> Dict:
        ...
        try:
            ...
            if diff_mean > 0:
                ...
                improvement_pct = ((mean_a - mean_b) / mean_b) * 100 if mean_b != 0 else 0
            else:
            ...
            return {
                ...
                "recommendation": self._get_metric_recommendation(
                    metric_name, is_statistically_significant, is_practically_significant, 
                    diff_mean, cohens_d
                )
            }
    ...
    def _get_metric_recommendation(self, metric_name: str, stat_sig: bool, pract_sig: bool, 
                                 diff: float, effect_size: float) -> str:
        """Generate recommendation for a specific metric"""
        
        if not stat_sig:
            return "No statistically significant difference detected. Continue monitoring."
        
        if not pract_sig:
            return "Statistically significant but not practically meaningful difference."
        ...
        if metric_name == 'crisis_detection_rate':
            ...
            if diff < 0:  # Lower is better for response time
                return f"Model B is {abs(diff)*1000:.0f}ms faster. Recommend deployment."
            else:
    ...
    def _calculate_power_for_effect_size(self, n1: int, n2: int, effect_size: float) -> float:
        ...
        try:
            ...
            power = 1 - stats.norm.cdf(critical_value - delta) + stats.norm.cdf(-critical_value - delta)
    ...
    def _calculate_required_sample_size(self, effect_size: float, desired_power: float = 0.8) -> int:
