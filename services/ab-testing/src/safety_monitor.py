...
from .models import SafetyViolation, SafetyThresholds
...
class SafetyMonitor:
    ...
    async def check_experiment_safety(self, experiment_id: str) -> bool:
        ...
        try:
            ...
            if not metrics:
                logger.warning("No metrics available for safety check", experiment_id=experiment_id)
            ...
            for model in ['a', 'b']:
                ...
                if crisis_rate is not None and crisis_rate < thresholds.crisis_detection_rate:
                ...
                if empathy_score is not None and empathy_score < thresholds.empathy_score:
                    violations.append(SafetyViolation(
                        timestamp=datetime.utcnow(),
                        experiment_id=experiment_id,
                ...
                if response_time_95th is not None and response_time_95th > thresholds.response_time_95th:
                    violations.append(SafetyViolation(
                        timestamp=datetime.utcnow(),
                        experiment_id=experiment_id,
            ...
            logger.error("Error in safety check", experiment_id=experiment_id, error=str(e))
    ...
    async def start_monitoring(self, experiment_id: str):
        ...
        while True:
            try:
                ...
                if not is_safe:
                    logger.critical("Safety violation - monitoring stopped", experiment_id=experiment_id)
                    break
                
                await asyncio.sleep(30)  # Check every 30 seconds
                
            except Exception as e:
                logger.error("Error in monitoring loop", experiment_id=experiment_id, error=str(e))
    ...
    async def get_safety_status(self, experiment_id: str) -> Dict:
        ...
        if experiment_id in self.violations:
            ...
            recent_violations = [
                v for v in self.violations[experiment_id]
                if v.timestamp > cutoff
            ]
        
        return {
            "is_safe": len([v for v in recent_violations if v.severity == 'critical']) == 0,
    ...
    async def _send_critical_alert(self, experiment_id: str, violations: List[SafetyViolation]):
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error("Failed to send alert", status_code=response.status_code)
