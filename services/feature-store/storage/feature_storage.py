...
class FeatureStorage:
    ...
    async def read_feature_values(
        ...
        try:
            ...
            if settings.COMPUTE_ENGINE == "duckdb":
                ...
                filters = []
                if entity_ids:
                    entity_filter = " OR ".join([f"entity_id = '{eid}'" for eid in entity_ids])
    ...
    async def materialize_features(
        ...
        try:
            ...
            source_data = await self._get_source_data(
                feature_set, start_date, end_date
            )
            ...
            output_path = await self.write_feature_values(
                feature_set, transformed_data
            )
    ...
    async def _update_online_store(
        self,
        feature_set: FeatureSet,
        data: pd.DataFrame
    ):
