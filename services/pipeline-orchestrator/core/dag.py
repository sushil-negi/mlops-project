...
class Task(BaseModel):
    ...
    operator: str = Field(..., min_length=1)  # Type of operator (data_ingestion, model_training, etc.)
    ...
    downstream_tasks: List[str] = Field(default_factory=list)  # Tasks that depend on this
    ...
    condition: Optional[str] = Field(default=None)  # Python expression for conditional execution
    trigger_rule: str = Field(default="all_success")  # all_success, all_done, one_success, etc.
    ...
    def validate_trigger_rule(cls, v):
        ...
        valid_rules = [
            "all_success", "all_done", "all_failed", "one_success", 
    ...
    def can_run(self, completed_tasks: Set[str], failed_tasks: Set[str]) -> bool:
        ...
        if self.status in [TaskStatus.RUNNING, TaskStatus.SUCCESS, TaskStatus.CANCELLED]:
            return False
        ...
        if self.trigger_rule == "all_success":
            return len(upstream_completed) == len(self.upstream_tasks) and not upstream_failed
        elif self.trigger_rule == "all_done":
            return len(upstream_completed) + len(upstream_failed) == len(self.upstream_tasks)
...
class Pipeline(BaseModel):
    ...
    def _find_orphaned_tasks(self) -> List[str]:
        ...
        for task_id, task in self.tasks.items():
            if not task.upstream_tasks and not task.downstream_tasks and len(self.tasks) > 1:
    ...
    def get_root_tasks(self) -> List[str]:
        ...
        return [task_id for task_id, task in self.tasks.items() if not task.upstream_tasks]
    
    def get_leaf_tasks(self) -> List[str]:
        """Get tasks with no downstream dependencies (leaf tasks)"""
        return [task_id for task_id, task in self.tasks.items() if not task.downstream_tasks]
    
    def get_runnable_tasks(self, completed_tasks: Set[str], failed_tasks: Set[str]) -> List[str]:
    ...
    def _calculate_path_duration(self, task_id: str, visited: Optional[Set[str]] = None) -> float:
        ...
        for upstream_id in task.upstream_tasks:
            upstream_duration = self._calculate_path_duration(upstream_id, visited.copy())
