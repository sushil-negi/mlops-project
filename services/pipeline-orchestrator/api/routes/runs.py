...
async def get_pipeline_run_metrics(run_id: str) -> Dict:
    ...
    return {
        ...
        "task_metrics": {
            ...
            "completed_tasks": len([t for t in run.task_executions if t.status == "success"]),
            "failed_tasks": len([t for t in run.task_executions if t.status == "failed"]),
            "pending_tasks": len([t for t in run.task_executions if t.status == "pending"])
        },
        "performance": {
