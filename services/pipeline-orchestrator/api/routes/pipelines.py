...
from core.dag import Pipeline, Task, ResourceRequirements, RetryPolicy, TriggerType
...
async def create_pipeline(pipeline_data: PipelineCreateRequest) -> PipelineDetailResponse:
...
async def validate_pipeline(pipeline_id: str) -> Dict:
    ...
    if total_cpu > 50:
        warnings.append("High total CPU requirements - consider optimizing resource usage")
    
    # Check for long-running tasks
    long_tasks = [task.name for task in pipeline.tasks.values() 
                 if task.resources.timeout_seconds > 7200]  # 2 hours
    if long_tasks:
