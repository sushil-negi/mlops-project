...
async def get_metrics(db: databases.Database = Depends(get_database)) -> Dict[str, Any]:
    ...
    try:
        ...
        db_result = await db.fetch_one("SELECT COUNT(*) as version_count FROM model_versions")
        version_count = db_result["version_count"] if db_result else 0
        
        db_result = await db.fetch_one("SELECT COUNT(*) as experiment_count FROM experiments")
        experiment_count = db_result["experiment_count"] if db_result else 0
        
        db_result = await db.fetch_one("SELECT COUNT(*) as artifact_count FROM artifacts")
