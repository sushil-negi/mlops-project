...
class VisualizationRequest(BaseModel):
    ...
    experiment_ids: Optional[List[uuid.UUID]] = Field(None, description="Experiment IDs to visualize")
    ...
    parameter_filters: Optional[Dict[str, Any]] = Field(None, description="Parameter filters")
...
class DashboardRequest(BaseModel):
    ...
    experiment_ids: Optional[List[uuid.UUID]] = Field(None, description="Experiment IDs")
...
async def create_visualization(viz_request: VisualizationRequest) -> VisualizationResponse:
...
@router.post("/charts/from-template/{template_name}", response_model=VisualizationResponse)
async def create_visualization_from_template(
    ...
    viz_request.visualization_type = template.get("type", viz_request.visualization_type)
...
async def get_dashboard(dashboard_id: uuid.UUID) -> DashboardResponse:
    ...
    dashboard = DashboardResponse(
        ...
        columns=2,
        widgets=[
            {"type": "metric_card", "metric": "accuracy", "position": {"row": 0, "col": 0}},
            {"type": "line_chart", "metrics": ["accuracy", "f1_score"], "position": {"row": 0, "col": 1}},
            {"type": "confusion_matrix", "metric": "predictions", "position": {"row": 1, "col": 0}},
            {"type": "bar_chart", "metric": "crisis_detection_rate", "position": {"row": 1, "col": 1}}
        ],
        created_at=datetime.utcnow(),
        updated_at=datetime.utcnow(),
        dashboard_url=f"/api/v1/visualizations/dashboards/{dashboard_id}",
...
async def export_experiment_visualizations(experiment_id: uuid.UUID, format: str = "pdf"):
    ...
    dashboard_data = {
        ...
        "widgets": [
            ...
            },
            {
                ...
                "data": {
                    "timestamps": ["2024-01-15T10:00:00Z", "2024-01-15T11:00:00Z", "2024-01-15T12:00:00Z"],
            ...
            },
            {
                ...
                "title": "Recent Alerts",
                "alerts": [
                    {"level": "info", "message": "Model accuracy stable at 98.2%", "time": "2 hours ago"},
                    {"level": "warning", "message": "Response time increased by 15ms", "time": "4 hours ago"}
                ]
            }
        ],
