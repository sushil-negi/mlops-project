...
from models.experiment import (
    ExperimentCreate, ExperimentUpdate, ExperimentResponse, ExperimentSummary,
    ExperimentListResponse, ExperimentComparison, ExperimentMetrics,
    ExperimentStatus, ExperimentType, EXPERIMENT_TEMPLATES
)
...
async def update_experiment(experiment_id: str, experiment_update: ExperimentUpdate) -> ExperimentResponse:
    """Update experiment"""
...
async def compare_experiments(comparison_request: ExperimentComparison) -> ExperimentComparison:
    ...
    comparison_result = ExperimentComparison(
        ...
        metric_comparison={
            "accuracy": {
                ...
                comparison_request.experiments[1]: 0.9756 if len(comparison_request.experiments) > 1 else 0.0
            },
            "f1_score": {
                comparison_request.experiments[0]: 0.9756,
                comparison_request.experiments[1]: 0.9689 if len(comparison_request.experiments) > 1 else 0.0
            }
        },
...
async def get_experiment_templates():
    ...
    return {
        ...
        "total": len(EXPERIMENT_TEMPLATES)
    }
